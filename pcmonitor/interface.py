...
class CoreCanvas(QWidget):
    ...
    def update_core(self, core, freq):
        """
        Atualiza a frequência e a temperatura associada ao núcleo.
        """
    ...
    def get_color_for_frequency(self, freq):
        """
        Retorna a cor de fundo com base na frequência, com uma maior variação de cores.
        """
        if freq < 1000:
            return QColor("#003d5c")  # Frequência muito baixa (Azul escuro)
        elif freq < 1200:
            return QColor("#00587a")  # Frequência baixa (Azul)
        elif freq < 1500:
            return QColor("#00796b")  # Frequência baixa a média (Verde escuro)
        elif freq < 1800:
            return QColor("#4caf50")  # Frequência média (Verde)
        elif freq < 2000:
            return QColor("#8bc34a")  # Frequência média alta (Verde claro)
        elif freq < 2200:
            return QColor("#fdd835")  # Frequência alta (Amarelo)
        elif freq < 2500:
            return QColor("#ff9800")  # Frequência muito alta (Laranja)
        elif freq < 2800:
            return QColor("#f44336")  # Frequência extremamente alta (Vermelho)
        else:
            return QColor("#c62828")  # Frequência crítica (Vermelho escuro)

    def get_temperature_for_core(self, core):
        """
        Retorna a temperatura do núcleo.
        """
        try:
            temps = psutil.sensors_temperatures().get('coretemp', [])
            if temps:
                # Encontrando a temperatura do núcleo específico
    ...
    def paintEvent(self, event):
        ...
        for x, y, w, h, core in rects:
            ...
            # Desenha a temperatura dentro do núcleo
...
if __name__ == "__main__":
    input_params = [[1000, 3000, [0, 1], 1]]  # Defina os parâmetros de frequência
